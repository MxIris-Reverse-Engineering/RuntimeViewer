...
public final class SidebarRuntimeObjectBookmarkViewModel: SidebarRuntimeObjectViewModel, @unchecked Sendable {
    public override init(imageNode: RuntimeImageNode, appState: AppState, router: any Router<SidebarRuntimeObjectRoute>) {
        super.init(imageNode: imageNode, appState: appState, router: router)
    ...
    override func buildRuntimeObjects() async throws -> [RuntimeObject] {
        appDefaults.objectBookmarks.filter { $0.source == appState.runtimeEngine.source && $0.object.imagePath == imagePath }.map { $0.object }
    }
