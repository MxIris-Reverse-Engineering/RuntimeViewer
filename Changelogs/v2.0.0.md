...
- Infinite loading when opening multiple document windows
- Crash when reconnecting XPC connections
- Catalyst side-loading failure
- Bonjour server reconnection after client restart
- Sidebar reentrant-reload crash (nested AppKit calls)
- Window title not reflecting the selected image
- Tokenizer destroying existing token attachments in the interface viewer
- Case-insensitive search toggle not taking effect
- Filter results not refreshing after switching search modes
- arm64e crash in code-injection environment
- Concurrent-access race in `RuntimeClientConnection` (serialized with `@Mutex`)
- Receive-loop deadlock caused by conflicting no-response handler sentinels
