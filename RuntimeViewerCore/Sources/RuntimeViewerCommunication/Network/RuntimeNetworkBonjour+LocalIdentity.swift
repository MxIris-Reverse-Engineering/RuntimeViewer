...
/// Where this process's advertised identity comes from.
///
/// Split out from the TXT record keys because almost every value here carries a
/// hazard the keys do not: an injected payload must not persist anything into
/// the host process, a device identifier must stay stable across processes, and
/// the host name has two forms because the obvious one performs a blocking
/// reverse-DNS lookup that has tripped a launch watchdog.
extension RuntimeNetworkBonjour {
