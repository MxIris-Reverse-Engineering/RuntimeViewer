        ...
        case .c,
            ...
            if let interface = try? await objcSection?.interface(for: name, using: options.objcHeaderOptions) {
                ...
                case .objc(.type(let kind)):
                    switch kind {
                    case .class:
                        return try? await objcSectionFactory.section(for: .class(name.name))?.interface(for: name, using: options.objcHeaderOptions)
                    case .protocol:
                        return try? await objcSectionFactory.section(for: .protocol(name.name))?.interface(for: name, using: options.objcHeaderOptions)
                    }
                default:
                    break
                }
