...
**日期**：2026-08-23
**相关提案**：[0013 支持注入 iOS Simulator 进程](../Evolutions/0013-inject-ios-simulator-process.md)
...
另有两项纵深防御尚未实现，都在 `swift-helper-service`：daemon 侧的平台守卫，
以及「模拟器目标拒绝 dlopen 时不回退 remap」。当前 dlopen 路径走得通、回退不触发，
所以它们不是阻塞项 —— 但一旦触发，remap 会用宿主地址打崩目标。
