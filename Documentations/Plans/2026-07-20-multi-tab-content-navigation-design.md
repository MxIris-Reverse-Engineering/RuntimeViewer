...
## Phase 1 落地后修复的竞态（2026-07-21）

上线后排查关 tab 崩溃时发现三个问题，均已修复，记录成因备查：

1. **`TabsControl` 关闭后的合成 selection 事件覆盖路由决策**。`TabsControl`（KPCTabsControl 血统）是自我变更型控件：点击 × 后它先自行移除按钮，`didCloseItem` delegate 返回后**同一调用栈内**继续执行内部善后——若被关的是选中 tab，它按自己的规则选**左**邻居并触发 `tabsControlDidChangeSelection`。而快照回写是异步的（嵌套发射时 RxSwift `MainScheduler` 会 dispatch async），此刻 `isApplyingSnapshot == false`，这个合成事件被当成用户点击转成了假 `.switchTab`，把 `.closeTab` 路由刚做出的**右**邻居激活覆盖掉，并造成 content/inspector 双重重建。修复：`TabBarAccessoryController` 增加 `isHandlingClose` 标志，`didCloseItem` 时置位、下一个 runloop pass 清除（合成 selection 与 close 严格同栈，异步清除必然晚于它、早于任何真实点击；不在 `applySnapshot` 里清除是为了路由拒绝、快照不回来时不吞掉用户的下一次点击）。教训：**自我变更型控件接到快照单向数据流上时，控件在「状态已变、快照未到」窗口内发出的一切派生事件都必须屏蔽**。
2. **`[unowned self]` + `Observable.async` 在 ViewModel 释放竞态下必崩**。`ContentTextViewModel` / `InspectorClassViewModel` 的 Rx 链在 `Task` 内以 unowned 载入 `self`；dispose 只能协作式取消 Task，关 tab / 切 tab 高频重建 ViewModel，in-flight 生成任务在 `deinit` 后执行 unowned load 即 `swift_abortRetainUnowned`。上面第 1 条的双重重建把「刚创建就被替换」变成常态，使该崩溃在关 tab 时近乎必现。修复：属性捕获（`[documentState = self.documentState, ...]`）或 `[weak self]` + guard；约定已写入 CLAUDE.md「Closures & Self Capture」。
3. **关闭非活动 tab 也全量重建 content/inspector**。`MainCoordinator.fanOut` 的 `case .switchTab, .closeTab` 不区分被关 tab 是否活动，可见对象未变也触发 `.back` 重建与 interface 重新生成。修复：`ContentCoordinator` 记录 `boundRuntimeObject`，`.back` 重入且对象未变时跳过 rebind（`.root` / `.next` 显式选择仍无条件重建，保留「重点同一行从生成失败中恢复」的语义）。

同批落地的两项遗留（2026-07-21）：

- **Open in New Tab 入口（Phase 2 前两处）**：Content 类型链接 ⌘⇧-click（⌘-click 原地跳转不变）+ 右键菜单 "Open in New Tab"（`RuntimeObjectMenuItem` 泛化自原 `JumpToDefinitionMenuItem`）；Sidebar 行右键菜单（基类 `SidebarRuntimeObjectViewController` 装配，Bookmark 变体自动继承；因基类是泛型类不能挂 `@objc` action，改用 RxAppKit 的 `menu.rx.items` + `itemSelected`，`menuNeedsUpdate` 同步按 `clickedRow` 重建 items，空 items 时 AppKit 不弹空菜单）。两条入口都走各自 ViewModel 的 `Input.runtimeObjectOpenedInNewTab` → `.openInNewTab` 路由；UIKit 端调用点传 `.empty()`。Inspector relationships 右键（设计标注可选）与 Duplicate Tab / 拖拽排序仍留在 Phase 2。
- **⌘W 动态键位**：`TabMenuController` 成为 File menu 的 `NSMenuDelegate`,`menuNeedsUpdate` 里按 `NSApp.target(forAction: closeTab)` 是否可达重排键位——document 窗口 key 时 Close Tab = ⌘W / Close Window = ⌘⇧W；其它窗口（Settings 等）key 时 Close Tab 摘除键位、Close Window 收回 ⌘W。AppKit 在解析 key equivalent 前会完整走一遍该 delegate 更新,所以快捷键路径与可见菜单一致。
