...
**第一版预热根本没跑(2026-08-16,Time Profiler 实测)。** 用户复测仍然卡,trace 显示 81ms 的
`dlopen` **整条在主线程上**,且没有任何工作线程在做同一件事。触发点是
`ContentCoordinator.desiredEditorKind()` → `SourceEditorLoader.isAvailable`,比 `lazy var bridge`
更早:决定用哪个编辑器就得先知道能不能用。

**真因是设置读得太早。** `SettingsLifecycleController.loadOnLaunch()` 是
`Task { await settings.load() }`,而第一版预热在 `applicationDidFinishLaunching` 里**同步**读
`settings.editor.usesSourceEditor` —— 那时存储还没读回来,拿到的是默认值,而编辑器主开关**默认关**,
于是 `guard isEnabledByUser` 直接 return,一个框架都没加载。

这条差点被误判成调度问题:第一版写的是 `Task.detached(priority: .utility)` 并挂在
`applicationDidFinishLaunching` 末尾,「启动最忙、后台优先级被推迟」听上去足以解释,
把它提前并升到 `userInitiated` 也确实是对的改动 —— **但那不是病因**。
分辨两者的证据就在 trace 里:预热若只是**开始得晚**,应当能在某个工作线程上看到它自己那份
`dlopen`;**一个都没有**,说明它压根没启动。

因此改成观察设置而不是读一次:`startPrewarmingWhenEnabled()` 用 `SwiftNavigation.observe`
盯 `settings.editor.usesSourceEditor`,`SettingsStore` 是 `@Observable` 且 `value` 是被追踪的属性,
`load()` 末尾替换 model 会重新触发。附带好处是用户在设置里**当场打开**开关也会立刻预热,
而不必等下次启动。
...
| 2026-08-20 | 关掉右键菜单里的 AutoFill | 那一项不是 SourceEditor 加的——"AutoFill" 这个词在 SourceEditor / SourceModelSupport / IDESourceEditor / DVTSourceEditor 四个二进制里一次都没出现。AppKit 在菜单打开时经 `NSContextMenuStartPluginsForMenuRef2` 注册一组 menu updater（Services、Continuity、AutoFill、Writing Tools），AutoFill 那个由 `_NSServicesInsertContextMenuUpdater` 插入 `+[NSMenuItem _standardAutoFillMenuItem]`；时机在 `contextMenu()` 把菜单交给 item provider 之后，所以 `setupContextMenu(for:)` 里删不掉。唯一的缝是守卫它的 `NSAutoFillSystemInsertMenuEnabled()`，而它走 `_NSGetBoolAppConfig` 读 `UserDefaults.standard`。故在 `applicationDidFinishLaunching` 里 `register(defaults:)` 写 registration domain（不落用户 plist），必须早于第一次右键——该函数把结果缓存在静态字节里只读一次。`-[NSApplication _addTextInputMenuItems:]` 用同一个开关控制 Edit 主菜单里的同名项，一并消失。**要连 Services / Continuity / Writing Tools 一起关**，则改注册外层的 `NSAddServicesToContextMenus = false`——那四个 updater 全在它的条件里；Writing Tools 没有自己的独立开关。两个 key 都无文档，读自 AppKit（macOS 26.5.2），细节见 `SystemAutoFillMenuSuppression` 的文件注释。 **该行记录的方案实测无效——只关 AutoFill 那个 key 注册得太晚，见下一条更正。** |
| 2026-08-20 | ⌥⌘-click 也开新 tab | 原先只有 ⌘⇧-click 开新 tab，现在 ⌥⌘-click 同义，两个 pane 一致。不必改 bridge：`SourceEditorView.mouseDown(with:)`（0x2A5DEC）**第一件事就是遍历 `eventConsumers`**，命中即 return，对修饰键没有任何预处理，之后才轮到 `selectionController`——`enableCmdClickMultiCursor` 的检查其实在 selection controller 内部，即在 consumer 之后（`CommandClickNavigator` 的注释说它"consumes it before any event consumer is offered the event"，措辞不准，但要设 false 的结论不变）。所以 ⌥⌘ 与 ⌘ 走同一条路，只需在 app 侧的分支里加一个修饰键。`NSTextView` pane 同步加了 `isPressedOption`，两条路径的判定收进 `isPressedOpenInNewTabModifier`。 |
| 2026-08-20 | 更正：只关 AutoFill 那个 key 注册得太晚 | 实测：在 `applicationDidFinishLaunching` 里 `register(defaults: ["NSAutoFillSystemInsertMenuEnabled": false])` **不起作用**，AutoFill 照常出现；改注册 `NSAddServicesToContextMenus = false` 才去掉。原因不是 key 选错，是读取时机：该 key 有两个读者，早的那个在启动流程里——`NSApplication.setMainMenu:` / `finishLaunching` / `_postDidFinishNotification` → `_customizeMainMenu` → `_addTextInputMenuItems:`，那是给 Edit 主菜单加 AutoFill / Emoji & Symbols / Dictation 的地方，全部早于任何 delegate 回调；而 `_NSGetBoolAppConfig` 第一次读完就把结果 `atomic_store` 进静态字节，此后永不再查 defaults，所以我们注册时值已定死为 YES。`NSAddServicesToContextMenus` 没有这样的早期读者——`xrefs` 只有 `NSContextMenuStartPluginsForMenuRef2` 与 `NSContextMenuEndPluginsForMenuRef2`，都在上下文菜单开合时运行——所以同一位置注册就生效。代价是 Services / Continuity / Writing Tools 一并消失，Edit 主菜单里的 AutoFill 仍在（它只受窄 key 控制）。要让窄 key 生效得在 `setMainMenu:`（主 nib 加载）之前注册，比任何 `NSApplicationDelegate` 回调都早，未尝试。类型随之更名为 `SystemContextMenuAdditionsSuppression`。 |
| 2026-08-20 | 再更正：改在 `AppDelegate.init` 注册，只关 AutoFill | 上一条判定"窄 key 注册得太晚"成立，但结论下早了——太晚的是 `applicationDidFinishLaunching`，不是所有时机。`AppDelegate` 由 `MainMenu.xib` 解档而来，而 nib 先实例化对象、后连接 outlet（`NSApplication.mainMenu` 即其一），所以 `init` 跑在 `setMainMenu:` → `_customizeMainMenu` → `_addTextInputMenuItems:` 那次读取**之前**。对照实验以"AppKit 有没有把 AutoFill 项放进 Edit 菜单"为判据（与右键那一项同一个 key）：在 `init` 注册 → 不出现；不注册 → 出现；在 `applicationDidFinishLaunching` 注册 → 出现。于是撤掉 `NSAddServicesToContextMenus`，Services / Continuity / Writing Tools 全部回来，Edit 主菜单里的 AutoFill 也一并消失。**这个调用不能挪进任何生命周期方法**，挪了会静默失效、唯一症状是菜单项回来——代码里两处注释都写了这一点，AGENTS.md 的 AppDelegate 约定也补了 `override init()` 的例外条款。关整组的办法保留在 `SystemAutoFillMenuSuppression` 的文件注释里。 |
| 2026-08-31 | 换 data source 之前必须先展开折叠 | 折叠区存在 `FoldingController` 上，而它属于 view（`SourceEditorView.foldingController` 是 stored `let`），不属于 data source：`dataSource` 的 setter 在折叠这一侧只调 `FoldingController.dataSource` 的 didSet（0x51E88），那个 didSet 只重建 delimiter data（+0x30），从不触碰 `allFolds`（+0x68）。Xcode 一个文档一个编辑器实例所以踩不到，本 bridge 复用同一个 view，上一份接口的折叠区就活到了下一份里——新行更短时 `FoldedRegionDisplay.visibleColumnRanges`（0x384D4）构造 `foldEnd.column ..< lineLength` 上下界颠倒，在 0x38788 触发 Swift 前置条件，**进程直接终止**，且发生在侧边栏点击自己的事件跟踪循环里。修复是 `setSource` 在赋值前调 `foldingController.unfoldAll(animate: false)`：必须在赋值前，因为它触发的重排要跑在折叠区所属的那个 data source 上；必须 `animate: false`，因为只有同步分支会在返回前清 `allFolds` 并把 `_cascadedFoldedRanges` 缓存（+0x70）置 nil。折叠列表为空时是 no-op，可以每次都调。与 Xcode 版本无关——崩溃点前后 32 字节在 26.5 / 26.6 逐字节相同，27.0 Beta 6 同名函数仍在。stub 相应新增 `FoldingController`（原生 Swift 类）、`unfoldAll(animate:)`（dispatch thunk）与 `SourceEditorView.foldingController`（direct symbol）共 2 个符号。详见 ResolvedIssues 同名纪要。 |
| 2026-08-31 | 补上复现测试，app 工程有了第一个 test target | `RuntimeViewerSourceEditorBridgeTests`（无宿主 unit test bundle），两个测试：一个断言换 source 后折叠列表为空，一个走完整的「折叠 → 换更短的文本 → 布局」。红绿都实测：撤掉 `unfoldAll` 那行，bundle 退出码 133（SIGTRAP）；恢复后 2 passed、退出码 0。**离屏就够**——崩溃要的是 layer-backed view 的 `layoutSublayers(of:)` 跑在 CA 事务里，不是可见窗口，也不需要 app 宿主。测试驱动的是 bridge 而不是裸 `SourceEditorView`，否则受测的顺序（`setSource` 里的先后）根本没被覆盖。第二个测试失败的方式是杀死 runner（`EXC_BREAKPOINT` 捕获不了），所以断言那个排在前面，先给一句能读的诊断。stub 为此多了 4 个符号（`fold(lineRanges:)`、`foldedRanges(containing:)` 与 `FoldingController` 的两个元数据访问器），仅测试使用，已在接口里注明。工程文件是手写的：本会话无 Xcode MCP，`xcodeproj` gem 又打不开 `objectVersion = 90`（不认数组形式的 `shellScript` 与新的 `dstSubfolder`），故照抄同样手工添加的 bridge target 结构、用相邻的 `E95ED17F` 标识符段，并以 `plutil` + `xcodebuild -list` 两道校验。**走 scheme 跑不通**——`xcodebuild test -scheme "RuntimeViewer macOS"` 要先构建整个 app，而那一步缺 `RuntimeViewerMobileServer.framework`；测试无宿主，直接 `xcrun xctest` 跑 bundle 即可。 |
...
| 2026-09-16 | 加 Settings › Editor › Xcode 选择器，显式选择压过内嵌副本 | `Settings.Editor.sourceEditorXcodePath` 存 `.app` 路径，空串为 Automatic。`XcodeSourceEditorLocator` 增加 `installedXcodes()`（走 LaunchServices，能找到版本管理器放在 `/Applications` 之外的副本）与 `frameworksDirectory(preferring:)`。**显式选择排在内嵌副本之前** —— 用户点了名还被换成别的，正是这个设置要防的那件事；Automatic 时维持原顺序。存路径而不是版本号：版本终究要解析回 bundle，且同一版本可以装多份。选中的副本缺框架时设置页就地提示，而不是拒绝选择。 |
| 2026-09-16 | 换 Xcode 必须重启，设置页直说并给按钮 | 框架一个进程只能加载一次，没有卸载路径。设置页用 `XcodeSourceEditorLocator.loadedFrameworksDirectory()`（扫 dyld 已加载镜像）拿到**本进程实际加载自哪里**，与当前选择解析出的目录比对，不一致才显示重启提示 —— 这样"Automatic 和显式选择指向同一份"时不会白提示一次。重启逻辑原本是 `HelperServiceVersionChecker` 的 private 方法，抽成 `RuntimeViewerSettings` 的 `ApplicationRelauncher` 依赖，两处共用一份实现。 |
| 2026-09-26 | 编辑器的滚动条改为自动隐藏；设一次即可，框架不会改回去 | `NSScrollView` 默认 `autohidesScrollers = false`，滚动条样式为「始终显示」时，内容放得下也会留一条空的竖向滚动槽。`SourceEditorView.scrollView` 在 ObjC 头里是 `@property (nonatomic, strong) SourceEditorScrollView *scrollView`（26.6 getter IMP `0x3F4FE0`），stub 以 `@objc dynamic` 声明、类型写成它的父类 `NSScrollView`，走 `objc_msgSend`，不引入任何新符号，`.tbd` 不用动；bridge 在 `init` 里设一次。**「设一次就够」是反编译得出的**（Xcode 26.6，二进制 UUID 与 dump 一致）：`setAutohidesScrollers:` 的 selref（`0x5BE9D8`）只被桩函数 `_objc_msgSend$setAutohidesScrollers:`（`0x47D500`）引用，桩只有一个调用点，在 `SourceEditorGoToListView.init(session:theme:)`（`0x140698`），与编辑器本身无关；`installScrollView()`（`0x3D61B8`）只设 `hasVerticalScroller` 等，不碰它；它注册的 `NSPreferredScrollerStyleDidChangeNotification` 观察者（closure #3，`0x3F5200`）只读 `scrollerStyle` 重算 `lineAnnotationInsetForVerticalScroller`。27.x 没有 dump 也没有反编译，改由测试兜底：`SourceEditorScrollerTests` 两条，一条断言 bridge 起来后为真，一条在换 source、发出滚动条样式变更通知后再断言一次。红绿都实测过（测试 bundle 的 rpath 指向 `/Applications/Xcode.app`，即 27.0）：撤掉那一行两条都失败、退出码 1，恢复后 8 条全过；`VerifyAcrossXcodes.sh` 在 26.6 / 27.0 / 27.1 / 27.2 上均能加载并跑完整个 bridge。 |
