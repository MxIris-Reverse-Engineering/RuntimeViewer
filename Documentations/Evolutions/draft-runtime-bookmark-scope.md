...
- **最后更新**: 2026-08-28
- **所属愿景**: 无
- **关联提案**: [0014](0014-inject-ios-simulator-process.md)（本提案处理它暴露出的持久化身份问题）
...
`Identifier`。0014 把客户端 `Identifier` 从对端服务名改成 `{deviceID}-{pid}`，pid 每次对端启动
...
各注入一个 SpringBoard —— 也就是 0014 的核心场景 —— 就会写同一个键。
...
`Equatable` / `Hashable`，目的正是让持久化身份与显示名解耦。0014 把 pid 塞回键里、把显示名绑回
...
- **旧对端的镜像引擎**（descriptor 缺字段）：先试着从 `name` 解析「设备名 (进程名)」形状；解析
  ...
  - **sidebar autosave 键**用 `source.description`（显示名）。**绝不能用 `source.identifier`**
    ——0014 之后 Bonjour client 的 identifier 含 pid，那样等于把 `PR106.10` 刚修掉的「每次启动换
...
是设备名、0014 之后是进程名），无条件当进程名用会把书签归到一个不存在的 scope —— 看上去迁移
...
不落盘，误判只影响当次会话。而且它在 `main` 上就存在，不是 0014 引入的。已登记在
...
**前置**：等 0014 的 PR #106 合入 `next`。本提案的身份构成建立在它把 deviceID 与 processName
...
28. **迁移与回退的判据被改写了。** 原文（「6. 迁移」）说 Bonjour 的旧键「只在 `name` 形如
    ...
    - 0014 之后写的键，`name` 是 `endpoint.processName ?? endpoint.name`，绝大多数情况就是**纯
      进程名**，不含括号 —— 会被这条规则全部丢弃，而这批恰恰是唯一能从 identifier 里取出
      deviceID 的、唯一迁得动的。
    - 0014 之前写的键，`name` 确实是「设备名 (进程名)」，但 identifier 是服务名，取不出
      deviceID，本来就迁不了。

    改为：**唯一的闸门是 identifier 能否被解析成 `{deviceID}-{processIdentifier}`**（那是构造
    scope 的硬前提，没有 deviceID 就没有 scope）。闸门一开就已经锁定了 `name` 的语义 —— 这种
    形状的 identifier 只可能是 0014 之后写的，此时 `name` 与运行时构造引擎用的是同一个字符串
