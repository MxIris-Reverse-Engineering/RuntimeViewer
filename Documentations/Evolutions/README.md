...
| [0022](0022-reveal-in-sidebar-navigator.md) | Reveal in Sidebar Navigator | Implemented | 仿 Xcode 的 Reveal in Project Navigator：新顶层菜单 Navigate 里的一条命令（⇧⌘J），在侧栏对象列表里选中内容区正在显示的对象并滚到可见处——必要时展开侧栏、切回对象列表分页、清掉挡住它的搜索词与 Filter Scope、展开分组与父节点，焦点移到列表。只在侧栏当前 image 内揭示，对象属于别的 image 时置灰。 |
| [0023](0023-content-loading-overlay.md) | 加载指示换成系统菊花，内容区加载改成 Xcode 式的整块遮罩 | Implemented | `CommonLoadingView` 从强调色圆环换成系统菊花（spinning `NSProgressIndicator`），背景照旧（macOS 26 以前毛玻璃、之后透明），所有面板一起换；内容区另给它编辑器背景色，成为盖住整个内容区（含 Minimap 与工具栏下方）的不透明遮罩。菊花在工具栏以下的可见区域居中、明暗跟背景色走；出现与消失的时机仍由 `delayedLoading` 决定。另加 Debug 专用的 Settings › Developer 页，可人为拖慢内容加载来测遮罩（存盘，由页面顶部的总开关统一开关）。 |
| [0024](0024-layer-backed-view-controller-adoption.md) | `BaseViewController` 改接 UIFoundation 的 `LayerBackedViewController` | Implemented | 基类从直接继承 AppKitPlus 的 `NSLayerBackedViewController` 改为继承 UIFoundation 的 `LayerBackedViewController<LayerBackedView>`（trait 开着，底下仍是同一个类），根视图换成 UIFoundation 的 `LayerBackedView`。UIFoundation 的 `contentView` 就是根视图，与原来那个内层容器重名，所以先把内层容器改名为 `containerView`（编译通过即改全），再换父类；内边距、加载遮罩层级、macOS 26 以前的毛玻璃都不变。 |
...
> **新提案从 `0024` 起编号**，不要只看本分支已有的文件挑下一个空号。
>
