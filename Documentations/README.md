...
见 [`Evolutions/README.md`](Evolutions/README.md)。当前 23 篇：Bonjour 可靠性、IDA 兼容导出、后台索引、泛型类型特化、DifferentiableBox 渲染范式、高基数 Cell ViewModel 的 Appearance 单流化、MCP Transport 绑定失败回收、ObjC 关系索引归还应用侧、ObjC 与 Swift 索引层对称化、内容视图编辑器选型、接口导出成图片、接入 UIFoundation Settings、用 AppKitPlus 取代 UXKit、支持注入 iOS Simulator 进程、构建阶段产出嵌入的 iOS-family 产物（已撤回）、RuntimeViewerApplication ViewModel 测试覆盖、`@Observed` 惰性创建 relay、`@RxObserved` 宏、Helper 设置页的重装按钮、App 图标按 Icon Composer 版本分两份文档、RuntimeObject 的相等性只表达身份、Reveal in Sidebar Navigator（在侧栏里选中并滚到当前对象）、加载指示换成系统菊花并把内容区加载改成 Xcode 式的整块遮罩；另有 10 篇待编号草案：本地运行时引擎搬进内嵌 XPC service（`.local` 身份不变，执行方式改为 XPC service）、RuntimeBookmarkScope（把持久化身份从显示名手里拿走）、给 `@objc @implementation` 实现的 ObjC 类加粉色角标、ObjC 类与 Swift 类互标角标并互相跳转、运行时对象列表的图标加 tooltip、后台索引给前台加载让路（后台在建时打开镜像不再被拖到后台建完），以及愿景《无头 RuntimeViewer》下的四篇——引擎管理下沉为无 UI 模块、`runtime-viewer-cli` 基础（协议 / 常驻 host / 本地来源）、多来源与 App 充当 host、嵌入 App 包与设置页。
...
- [折叠一段代码后切换类，SourceEditor 布局时崩溃](ResolvedIssues/2026-08-31-source-editor-stale-folds-crash.md)（2026-08-31）—— 折叠区属于 view 而非 data source，换 data source 不清空它，旧折叠区遇上更短的新行即触发 `Range` 前置条件。
